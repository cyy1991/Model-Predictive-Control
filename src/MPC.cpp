 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 0; i < N - 1; i++) {
      ...
      // Use previous acceleration and steering angle to account for latency
      AD<double> delta0 = vars[delta_start + (i == 0 ? i : i - 1)];
      AD<double> a0 = vars[a_start + (i == 0 ? i : i - 1)];
...
std::tuple<vector<double>, vector<double>, vector<double>> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // State vector + actuations for initial state
  std::vector<double> val = {solution.x[x_start + 1], solution.x[y_start + 1],
